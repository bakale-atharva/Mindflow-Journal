  ...
  return (
    <ol
      className={cn(
        ...
        !compact && "lg:grid-cols-1 lg:gap-1",
      )}
    ...
    >
      {days.map((day) => {
        ...
        if (isLocked) markerStyles = "border-ink/8 bg-white/40 text-ink/30";
          ...
          markerStyles = "border-lilac bg-orchid-mist text-ink";
        else if (isComplete)
          markerStyles = "border-seafoam bg-seafoam text-ink";
          ...
          <>
            ...
            {!compact ? (
              <span className="hidden min-w-0 lg:block">
                <span className="block font-mono text-[10px] uppercase tracking-[.12em] text-ink/40">
                  Day {day.day}
                </span>
                <span className="block truncate text-sm font-medium text-ink/75">
                  {day.theme}
                </span>
              </span>
            ) : null}
        ...
        const classes =
          "flex items-center justify-center gap-3 rounded-2xl p-1 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary lg:justify-start lg:p-2";
