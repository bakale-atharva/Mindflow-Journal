...
export function TodayView({ dashboard, activeDay, activeEntry }: { dashboard: DashboardData; activeDay: ProgramDayView; activeEntry: JournalEntry | null }) {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-5 sm:px-7 sm:py-8 lg:px-10">
      ...
      <div className="mb-6 rounded-[26px] border border-ink/8 bg-white/65 p-4 lg:hidden"><SevenDayPath days={dashboard.days} compact activeDay={activeDay.day} /></div>
      ...
      <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_280px]">
        <div className="space-y-6">
          ...
          <DayExperienceDispatcher key={`${activeDay.day}-${activeEntry?.updated_at ?? 'new'}`} day={activeDay} entry={activeEntry} />
        </div>

        <aside className="hidden space-y-5 lg:block">
          <section className="surface rounded-[26px] p-5"><div className="mb-4 flex items-center justify-between"><h2 className="font-heading text-lg font-semibold tracking-[-.03em]">Seven-day path</h2><span className="font-mono text-[10px] text-ink/35">{dashboard.entries.length}/7</span></div><SevenDayPath days={dashboard.days} activeDay={activeDay.day} /></section>
