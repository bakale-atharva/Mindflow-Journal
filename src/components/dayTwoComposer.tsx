...
export function DayTwoComposer({
  ...
  const responseData = entry?.response_data as
    | import("@/app/actions").DayTwoResponseData
    | null
    | undefined;
  const initialUrgent =
    responseData?.urgent ??
  ...
  function discardChanges() {
    if (
      (urgent === initialUrgent && canWait === initialCanWait) ||
      window.confirm("Discard the changes in this Day 2 exercise?")
    ) {
    ...
    <section className="surface rounded-[30px] p-5 sm:p-7">
      <div className="flex items-start justify-between gap-4 mb-4">
        <div>
          ...
          <p className="mt-2 text-sm leading-6 text-ink/60">
            Not everything asking for your attention needs an answer today.
            Place each thought where it belongs for now.
          </p>
        </div>
        {entry ? (
      ...
      <form action={action} className="mt-7">
        ...
        <div className="grid lg:grid-cols-2 gap-4 lg:gap-6 mb-6">
          <div className="rounded-xl border border-coral/30 bg-coral/5 p-4 transition focus-within:border-coral/50 focus-within:ring-4 focus-within:ring-coral/10 sm:p-5 flex flex-col">
            <label
              htmlFor="dayTwo-urgent"
            ...
            <textarea
              ...
              placeholder="Write the thought, task, decision, or conversation that feels pressing…"
          ...
          <div className="rounded-xl border border-seafoam/30 bg-seafoam/5 p-4 transition focus-within:border-seafoam/50 focus-within:ring-4 focus-within:ring-seafoam/10 sm:p-5 flex flex-col">
            ...
            <textarea
              ...
              maxLength={10000}
              value={canWait}
              onChange={(event) => setCanWait(event.target.value)}
              placeholder="Write what you can leave alone for now…"
              className="min-h-44 w-full flex-1 resize-none overflow-y-auto bg-transparent text-base leading-8 text-ink outline-none placeholder:text-ink/30"
            />
          </div>
        </div>
        ...
        <div className="mt-5 flex items-center justify-end gap-3">
          ...
          <button
            type="submit"
            disabled={pending || (!urgent.trim() && !canWait.trim())}
          ...
          >
            ...
            {pending ? "Saving Day 2…" : entry ? "Save changes" : "Save Day 2"}
          </button>
        </div>
      </form>
