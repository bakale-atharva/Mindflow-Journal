...
export function SettingsPanel({ profile }: { profile: ProfileState }) {
  ...
  const consentEnabled = Boolean(profile.ai_processing_consent_at)
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-6 sm:px-7 sm:py-10 lg:px-10">
      ...
      <div className="mt-8 space-y-5">
        <section className="surface rounded-[28px] p-6 sm:p-7"><div className="flex items-start gap-4"><span className="grid size-11 shrink-0 place-items-center rounded-2xl bg-orchid-mist text-primary"><Sparkles className="size-5" /></span><div><h2 className="font-heading text-2xl font-semibold tracking-[-.04em]">AI reflections</h2><p className="mt-2 max-w-xl text-sm leading-6 text-ink/55">When enabled, saved entry text is sent to OpenAI for one concise reflection. Turning this off does not remove or block your journal.</p></div></div><form action={consentAction} className="mt-6 flex flex-wrap items-center gap-3"><input type="hidden" name="ai_consent" value={consentEnabled ? 'no' : 'yes'} /><button disabled={consentPending} className="rounded-2xl bg-ink px-5 py-3 text-sm font-semibold text-white disabled:opacity-50">{consentPending ? 'Updating…' : consentEnabled ? 'Turn off reflections' : 'Turn on reflections'}</button><span className="rounded-full bg-porcelain px-3 py-2 text-xs font-semibold text-ink/48">Currently {consentEnabled ? 'on' : 'off'}</span></form>{consentState ? <p className={`mt-3 text-sm ${consentState.status === 'error' ? 'text-destructive' : 'text-ink/55'}`}>{consentState.status === 'error' ? consentState.error : consentState.message}</p> : null}</section>
