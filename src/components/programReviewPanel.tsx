...
export function ProgramReviewPanel({
  ...
  const [keepState, keepAction, isKeeping] = useActionState(
    keepProgramPractice,
    null,
  );
  ...
  const isStale =
    ...
    review.source_hash !== currentHash;
    ...
    return (
      <section className="surface mt-6 rounded-[32px] p-7 sm:p-10 flex flex-col items-center text-center">
        ...
        <p className="text-sm leading-6 text-ink/65 max-w-md mx-auto mb-6">
          ...
          and write a gentle, private reflection of the themes that emerged over
          the last seven days.
        </p>
  ...
  if (!review || (review.status === "failed" && review.attempt_count === 0)) {
    return (
      <section className="surface mt-6 rounded-[32px] p-7 sm:p-10 flex flex-col items-center text-center">
        ...
        <p className="text-sm leading-6 text-ink/65 max-w-md mx-auto mb-6">
          7 of 7 entries completed. Generate a private, synthesized reflection
          of your week.
        </p>
        ...
        <form action={genAction}>
          ...
          >
            {isGenerating ? (
              ...
              "Create my seven-day reflection"
            )}
          </button>
        </form>
    ...
    return (
      <section className="surface mt-6 rounded-[32px] p-7 sm:p-10 flex flex-col items-center text-center animate-pulse">
        ...
        <p className="text-sm leading-6 text-ink/65 max-w-md mx-auto">
          This may take a moment to gather the threads from your week.
        </p>
      </section>
    );
  }
  ...
  return (
    <section className="surface mt-6 rounded-[32px] p-7 sm:p-10">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h2 className="font-heading text-2xl font-semibold tracking-[-.02em] text-ink">
      ...
      {review.practice ? (
        <div className="rounded-2xl bg-orchid-mist/15 p-6 sm:p-8">
          ...
          <p className="text-sm leading-7 text-ink/75 mb-5">
            {review.practice}
          </p>
          ...
          {review.practice_kept_at ? (
            ...
            <form action={keepAction}>
              <button
                type="submit"
                disabled={isKeeping}
                className="inline-flex items-center gap-2 text-xs font-semibold text-ink px-4 py-2 rounded-xl border border-ink/10 hover:bg-ink/5 transition-colors disabled:opacity-50"
              >
                Keep this practice
              </button>
            </form>
      ...
      {isStale ? (
        <div className="mt-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4 rounded-2xl border border-ink/5 bg-white/40 p-4">
          <p className="text-xs text-ink/60">
            Your journal has been edited since this was generated.
          </p>
          <form action={retryAction}>
            <button
              type="submit"
              disabled={isRetrying}
              className="inline-flex items-center gap-2 text-xs font-semibold text-ink px-3 py-1.5 rounded-lg hover:bg-ink/5 transition-colors"
            >
