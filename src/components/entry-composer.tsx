  ...
  return (
    <section className="surface rounded-[30px] p-5 sm:p-7">
      <div className="flex items-start justify-between gap-4">
        <div><p className="font-mono text-[10px] uppercase tracking-[.16em] text-ink/42">Your private entry</p><h2 className="mt-2 font-heading text-2xl font-semibold tracking-[-.04em] text-ink">Write what is true right now.</h2></div>
      ...
      <form action={action} className="mt-7">
        ...
        <div className="mt-5 flex items-center justify-end gap-3">
          {content !== originalContent ? <button type="button" onClick={discardChanges} className="rounded-xl px-4 py-2.5 text-sm font-semibold text-ink/45 hover:bg-ink/5">Discard</button> : null}
          <button type="submit" disabled={pending || !content.trim()} className="inline-flex items-center gap-2 rounded-2xl bg-ink px-5 py-3 text-sm font-semibold text-white shadow-[0_12px_30px_rgba(36,33,53,.18)] disabled:cursor-not-allowed disabled:opacity-35"><Save className="size-4" />{pending ? 'Saving entry…' : entry ? 'Save changes' : 'Save entry'}</button>
        </div>
      </form>
