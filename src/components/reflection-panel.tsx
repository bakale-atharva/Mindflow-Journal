...
import { retryReflection, type AIReflection } from '@/app/actions'
...
export function ReflectionPanel({ entryId, reflection, generated }: { entryId: string; reflection?: AIReflection | null; generated?: { status: string; reflection: string | null; question: string | null } | null }) {
  ...
  if (status === 'failed') {
    return <div className="rounded-[22px] border border-ink/10 bg-white p-5"><p className="text-sm leading-6 text-ink/60">Your entry is saved, but the reflection could not be generated.</p><form action={retryAction} className="mt-4"><input type="hidden" name="entry_id" value={entryId} /><button disabled={pending} className="inline-flex items-center gap-2 rounded-xl bg-ink px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-50"><RotateCcw className="size-4" />{pending ? 'Retrying…' : 'Try once more'}</button></form>{retryState?.status === 'error' ? <p className="mt-3 text-sm text-destructive">{retryState.error}</p> : null}</div>
  }
