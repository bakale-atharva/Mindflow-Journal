...
export function DaySevenComposer({
  ...
  const responseData = entry?.response_data as
    | import("@/app/actions").DaySevenResponseData
    | null
    | undefined;
  const initialClearer =
    responseData?.became_clearer ??
  ...
  function discardChanges() {
    if (
      (becameClearer === initialClearer && carryForward === initialCarry) ||
      window.confirm("Discard the changes in this Day 7 exercise?")
    ) {
  ...
  const combinedLength =
    (becameClearer
      ? `Looking back, what became clearer:\n${becameClearer}\n\n`
      : ""
    ).length +
    (carryForward ? `What I want to carry forward:\n${carryForward}` : "")
      .length;
    ...
    <section className="surface rounded-[30px] p-5 sm:p-7">
      <div className="flex items-start justify-between gap-4 mb-4">
        <div>
          ...
          <p className="mt-2 text-sm leading-6 text-ink/60">
            A moment to gather what you&apos;ve found before moving forward.
          </p>
        </div>
        {entry ? (
      ...
      <form action={action} className="mt-7">
        ...
        <div className="max-w-2xl mx-auto flex flex-col mb-10 relative">
          <div className="relative z-10 rounded-t-[32px] rounded-b-none border border-b-0 border-ink/5 bg-porcelain shadow-[0_-8px_24px_rgba(36,33,53,0.02)] p-6 sm:p-8 flex flex-col group">
            ...
            <p className="text-xs text-ink/60 mb-5">
              Looking back on the past six days, what became clearer for you?
            </p>
            <textarea
              id="daySeven-clearer"
              ref={clearerRef}
              ...
              maxLength={10000}
              value={becameClearer}
              onChange={(event) => setBecameClearer(event.target.value)}
              placeholder="Write what you've noticed or realized…"
              className="w-full flex-1 resize-none bg-transparent text-base leading-8 text-ink outline-none placeholder:text-ink/30"
            />
          </div>

          <div className="h-px w-full bg-ink/10 relative z-20 shadow-[0_2px_8px_rgba(36,33,53,0.05)]"></div>
          ...
          <div className="relative z-10 rounded-b-[32px] rounded-t-none border border-t-0 border-ink/10 bg-orchid-mist/15 p-6 sm:p-8 transition-all duration-300 focus-within:bg-orchid-mist/25 flex flex-col group">
            ...
            <p className="text-xs text-ink/60 mb-5">
              What would you like to carry forward from here?
            </p>
            <textarea
              id="daySeven-carry"
              ref={carryRef}
              ...
              placeholder="Write a thought to take with you…"
        ...
        <div className="mt-5 flex items-center justify-end gap-3">
          ...
          <button
            type="submit"
            disabled={pending || !becameClearer.trim()}
      ...
      {state?.status === "success" ? (
        <div className="mt-8 flex flex-col items-center">
          ...
          <Link
            href="/journal"
            className="mt-8 inline-flex items-center gap-2 rounded-full bg-porcelain border border-ink/10 px-6 py-3 text-sm font-semibold text-ink hover:bg-ink/5 hover:border-ink/20 transition-all group"
          >
