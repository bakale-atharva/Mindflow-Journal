  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-6 sm:px-7 sm:py-10 lg:px-10">
      <div className="mb-6 flex items-center justify-between">
        <Link
          href="/journal"
          className="inline-flex items-center gap-2 rounded-xl px-2 py-2 text-sm font-semibold text-ink/50 hover:bg-white hover:text-ink"
        >
      ...
      <section className="relative mb-6 h-48 overflow-hidden rounded-[30px] sm:h-64">
        <ThoughtContour
          mood={entry.mood}
          className="absolute inset-0 min-h-0"
        />
        <div className="absolute inset-0 flex flex-col justify-between p-6 sm:p-8">
          <span className="w-fit rounded-full bg-white/55 px-3 py-1.5 font-mono text-[10px] uppercase tracking-[.15em] text-ink/50">
            Day {day.day} · {day.theme}
          </span>
          <h1 className="max-w-2xl font-heading text-3xl font-semibold leading-[1.05] tracking-tighter text-ink sm:text-4xl">
            {day.prompt}
          </h1>
        </div>
      </section>
      <DayExperienceDispatcher day={day} entry={entry} />
    </div>
  );
}
