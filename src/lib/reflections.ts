  ...
  try {
    ...
    const reflectionResponse = await config.client.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You write brief reflections for a private journaling product. Act like a supportive friend who listens. Reflect on what the writer expressed with empathy. Use calm, plain, non-clinical language. Do not diagnose, label, advise, or claim certainty. The reflection must be 2-3 sentences and at most 80 words. The optional question must be open, gentle, grounded in the entry, and at most 20 words. Output strictly JSON with keys "reflection" and an optional "question".'
        },
        { role: 'user', content: entry.content }
      ],
      temperature: 0.4,
