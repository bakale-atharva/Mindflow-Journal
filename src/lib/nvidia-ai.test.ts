...
test('uses NVIDIA defaults only when an API key exists', () => {
  withEnvironment({ NVIDIA_API_KEY: 'test-key' }, () => {
    ...
    assert.equal(config?.reflectionModel, 'openai/gpt-oss-20b')
...
test('parses JSON and requires explicit NVIDIA version-4 consent', () => {
  ...
  assert.equal(hasActiveNvidiaConsent({ ai_processing_consent_at: '2026-07-16T00:00:00Z', ai_processing_consent_revoked_at: null, ai_processing_provider: 'nvidia', ai_consent_version: 4 }), true)
